...
var keyPressed = function() {
    ...
    if (keys[17] && keys[90]) { // Ctrl + Z
        ...
        drawingHistory.concat(points.splice(points.lastIndexOf(false)));
    }
    if (keys[17] && keys[89]) { // Ctrl + Y
        if (drawingHistory.length) points.concat(drawingHistory.splice(drawingHistory.lastIndexOf(false)));
    }
    savePoints();
};
